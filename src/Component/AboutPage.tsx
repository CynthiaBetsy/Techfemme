...
const AboutPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-300 to-purple-100 px-6 py-12 flex items-center justify-center relative">
      <div className="max-w-4xl w-full bg-purple-50 shadow-xl rounded-2xl p-10 space-y-6 z-10">
        <h1 className="text-4xl font-bold text-center text-purple-400">About TechFemme</h1>

        <p className="text-lg text-gray-700">
          At <span className="font-semibold text-purple-400">TechFemme Academy</span>, we believe that diversity drives innovation.
        ...
        <p className="text-xl text-purple-400 font-semibold text-center">
          Join the movement. Build your future.
        </p>

        <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto">
          {features.map(({ icon: Icon, label }) => (
            <li key={label} className="flex items-center gap-3 text-purple-400 font-medium">
              <Icon className="text-purple-400 w-7 h-7 shrink-0" />
              {label}
            </li>
          ))}
        </ul>
        ...
        <div className="flex flex-col items-center gap-4 pt-6">
          <p className="flex items-center text-lg font-medium text-gray-700">
            To get started with us <ArrowRight className="ml-2 text-purple-400" />
          </p>
          <button
            onClick={handleRegisterClick}  // Navigate to registration page
            className="px-6 py-3 bg-purple-400 text-white rounded-full shadow hover:bg-green-400 transition cursor-pointer"
          >
            Register
          </button>
        </div>
